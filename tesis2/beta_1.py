...
class WaterLevelCard(QFrame):
    def __init__(self, width=100, height=150):
        ...
        self.water.setFixedHeight(0)
        self.water.setMaximumWidth(width-10)
        ...
        self.anim_timer.timeout.connect(self.animate_level)
        self.anim_timer.start(50)

    def set_level(self, level_percent):
        self.target_level = max(0, min(100, level_percent))
    ...
    def animate_level(self):
        ...
        if self.level > 70:
            color = "#3b82f6"  # azul lleno
        elif self.level > 30:
            color = "#facc15"  # amarillo medio
        else:
            color = "#dc2626"  # rojo bajo
        self.water.setStyleSheet(f"background:{color}; border-radius:8px;")
...
class Dashboard(QWidget):
    ...
    def _init_ui(self):
        ...
        self.nivel_card = WaterLevelCard(width=100, height=150)  # animación
    ...
    def update_values(self, parsed: dict):
        ...
        if 'nivel' in parsed:
            ...
            if self.nivel.upper() == "OK":
                self.nivel_card.set_level(100)
            else:
                self.nivel_card.set_level(30)
