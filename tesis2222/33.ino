...
float leerTDS() {
  int lectura = analogRead(TDS_PIN);  // lectura instantánea
  float voltaje = lectura * VREF / 1024.0;
  float compensado = voltaje / (1.0 + 0.02 * (temperature - 25.0));

  float tdsInstant = (133.42 * compensado * compensado * compensado
                    - 255.86 * compensado * compensado
                    + 857.39 * compensado) * TDS_FACTOR;  // factor EC
