...
# ========================
def enviar_correo_excel(destinatario, asunto, mensaje, archivo_adjunto):
...
# ========================
# Función para enviar datos por correo en Excel
# ========================
def enviar_datos_cliente_excel():
    selected = tree.selection()
    if not selected:
    ...
    # Crear Excel con los datos del cliente
    df = pd.DataFrame([{
        "Nombre": nombre,
    ...
    archivo_excel = f"{nombre}_datos.xlsx"
    df.to_excel(archivo_excel, index=False)

    # Preparar mensaje y enviar correo con Excel adjunto
    asunto = f"Tus datos de registro, {nombre}"
    mensaje = f"Hola {nombre},\n\nAdjunto tus datos registrados en Excel."
    enviar_correo_excel(correo, asunto, mensaje, archivo_excel)
...
btn_enviar_correo = tk.Button(frame_btn, text="Enviar Datos por Correo", command=enviar_datos_cliente_excel, bg="orange", fg="white")
btn_enviar_correo.pack(side="left", padx=5)
