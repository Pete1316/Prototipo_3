...
# ========================
# Función para cargar productos desde la DB
# ========================
...
def agregar_producto():
    ...
    tree_productos.insert("", tk.END, values=(nombre, cantidad, precio, subtotal))
...
def generar_factura_pdf():
    ...
    productos = []
    for item in tree_productos.get_children():
        prod, cant, precio, subtotal = tree_productos.item(item)["values"]
        productos.append({
            "producto": prod,
            "cantidad": float(cant),
            "precio": float(precio),
    ...
    for p in productos:
        ...
        pdf.cell(30, 8, f"${p['subtotal']:.2f}", border=1, ln=True, fill=True)
        fill = not fill
    ...
    messagebox.showinfo("Correo enviado", f"Se envió la factura a {correo}")
...
# --- Frames principales (productos a la izquierda, cliente a la derecha) ---
...
tree_productos.grid(row=1, column=0, columnspan=5, pady=5)

# --- Contenido frame cliente ---
