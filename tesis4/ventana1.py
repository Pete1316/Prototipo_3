...
from PySide6.QtCore import QThread, Signal, Qt, QPropertyAnimation, QRect
...
class Ventana(QWidget):
    ...
    # --------------------------- CREAR TARJETA DE AGUA ANIMADA ---------------------------
    def crear_tarjeta_nivel_animado(self, titulo):
        frame = QFrame()
        frame.setStyleSheet("""
            background-color: #f9fafb;
            border: 1px solid #d1d5db;
        ...
        # Contenedor de agua
        nivel_contenedor = QFrame()
        nivel_contenedor.setStyleSheet("background-color: #e5e7eb; border-radius: 5px;")
        nivel_contenedor.setFixedHeight(100)
        nivel_contenedor.setFixedWidth(50)

        # Barra de agua
        barra_agua = QFrame(nivel_contenedor)
        barra_agua.setStyleSheet("background-color: #3b82f6; border-radius: 5px;")
        barra_agua.setGeometry(0, 100, 50, 0)  # Empieza vacío

        layout.addWidget(label_titulo, alignment=Qt.AlignCenter)
        layout.addWidget(nivel_contenedor, alignment=Qt.AlignCenter)

        frame.barra_agua = barra_agua
        frame.nivel_contenedor = nivel_contenedor
        return frame

    # --------------------------- ANIMACION DE NIVEL DE AGUA ---------------------------
    def animar_nivel_agua(self, valor):
        try:
            valor = float(valor)
        except:
            return
        # Valor esperado 0-100
        altura = int((valor / 100) * self.tarjeta_nivel.nivel_contenedor.height())
        nueva_y = self.tarjeta_nivel.nivel_contenedor.height() - altura

        anim = QPropertyAnimation(self.tarjeta_nivel.barra_agua, b"geometry")
        anim.setDuration(500)  # 0.5 segundos de animación
        anim.setStartValue(self.tarjeta_nivel.barra_agua.geometry())
        anim.setEndValue(QRect(0, nueva_y, 50, altura))
        anim.start()
    ...
    def agregar_cosas(self):
        ...
        self.tarjeta_nivel = self.crear_tarjeta_nivel_animado("Nivel Agua")
    ...
    def mostrar_datos(self, datos):
        ...
        if info:
            ...
            if 'Nivel_A' in info:
                self.animar_nivel_agua(info['Nivel_A'])
